...
PROJECT_API_NAMESPACE_PREFIX = '.spec.v1.'

# Namespace prefix for Envoy top-level APIs.
PROJECT_PREFIX = '.'

# Annotation Prefix
PROJECT_ANNOTATION_PREFIX = "api"
...
# TODO we have to change this to be more general
API_URL_FORMAT = 'https://github.com/lyft/flyteidl/blob/{}/protos/%s#L%d'.format(
    os.environ['REPO_BLOB_SHA'])
  ...
  def GithubUrl(self, path):
    ...
    for location in self._proto.location:
      if location.path == path:
        return API_URL_FORMAT % (self._name, location.span[0])
    return ''
...
def FormatHeader(style, text):
  ...
  """
  return '%s\n%s\n\n' % (os.path.basename(text), style * len(text))
...
def NormalizeFQN(fqn):
  ...
  Strips leading PROJECT_API_NAMESPACE_PREFIX and PROJECT_PREFIX and makes pretty wrapped type names.
  ...
  """
  if fqn.startswith(PROJECT_API_NAMESPACE_PREFIX):
    return fqn[len(PROJECT_API_NAMESPACE_PREFIX):]
  if fqn.startswith(PROJECT_PREFIX):
    return fqn[len(PROJECT_PREFIX):]
  return fqn
...
def FormatFieldType(type_context, field):
  ...
  """
  if field.type_name.startswith(PROJECT_API_NAMESPACE_PREFIX) or field.type_name.startswith(
      PROJECT_PREFIX):
...
def FileCrossRefLabel(msg_name):
  ...
  return '%s_file_%s' % (PROJECT_ANNOTATION_PREFIX, msg_name)


def MessageCrossRefLabel(msg_name):
  """Message cross reference label."""
  return '%s_msg_%s' % (PROJECT_ANNOTATION_PREFIX, msg_name)


def EnumCrossRefLabel(enum_name):
  """Enum cross reference label."""
  return '%s_enum_%s' % (PROJECT_ANNOTATION_PREFIX, enum_name)


def FieldCrossRefLabel(field_name):
  """Field cross reference label."""
  return '%s_field_%s' % (PROJECT_ANNOTATION_PREFIX, field_name)


def EnumValueCrossRefLabel(enum_value_name):
  """Enum value cross reference label."""
  return '%s_enum_value_%s' % (PROJECT_ANNOTATION_PREFIX, enum_value_name)
